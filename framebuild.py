...
-------------""")
    ...
    def render_jig_diagram(self, render=None, px_per_mm=2):
        fork_path = self._fork_path()

        if render is None:
            inf = array([self.seat_tube.tube_top[0], 0])
            sup = array([fork_path[-1][0], self.head_tube.tube_top[1]])
            r = Render(inf, sup, px_per_mm)
        else:
            r = render

        tubecolors = ['SteelBlue','Indigo','Fuchsia','black']
        for i,tube in enumerate([self.seat_tube, self.top_tube, self.head_tube,
                                 self.down_tube]):
            r.draw_tube(tube,fill=tubecolors[i])

        #for mitre in (self.tt_ht, self.dt_ht, self.dt_st, self.tt_st):
        #    r.draw_mitre(mitre)

        # Add a reference line
        ref_line_norm = array( [self.seat_tube.vecn[1],-self.seat_tube.vecn[0]])
        start1 = array([0,0])
        end1=700*ref_line_norm
        r.polyline((start1, end1), "black")

        # top reference line
        start2 = self.seat_tube.top[0:-1]
        end2 = start2 + end1
        r.polyline((start2, end2), "black")

        # head tube reference line
        ht_line_norm = array( [self.head_tube.vecn[0],self.head_tube.vecn[1]])
        startht = self.head_tube.top[0:-1]+200*ht_line_norm
        endht = startht-800*ht_line_norm
        r.polyline((startht, endht), "green")

        #intersections of ht line and ref lines
        intersection_ref1 = get_intersect(start1,end1,startht,endht)
        intersection_ref2 = get_intersect(start2,end2,startht,endht)
        r.draw_point(intersection_ref1,"black")
        r.draw_point(intersection_ref2,"black")
        ...
        ### PRINT SOME INFO
        print("""\nMM Jig Metrics\n-------------""")
        print("length along reference line BB = ",round(norm(intersection_ref1),2) )
        print("length along reference line TT = ",round(norm(intersection_ref2-start2),2) )
        print("length along HT line from BB reference = ", round(norm(intersection_ref1-self.head_tube.tube_bottom[0:-1]),2) )
        print("length along HT line from TT reference = ", round(norm(intersection_ref2-self.head_tube.tube_top[0:-1]),2) )


        #r.draw_point(self.head_tube.tube_top[0:-1],"yellow")
...
def main():
    ...
    jig = frame.render_jig_diagram(None, args.resolution)
    ...
    jig.save("side_view_jig.png")
    print("Saved jig diagram to side_view_jig.png")
